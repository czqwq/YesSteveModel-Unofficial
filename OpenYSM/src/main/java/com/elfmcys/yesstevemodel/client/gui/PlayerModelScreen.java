...
public class PlayerModelScreen extends Screen implements IGuiWidget {
    ...
    public ModelButton createModelButton(int x, int y, PlayerPreviewEntity previewEntity, ModelAssembly modelAssembly) {
        return new ModelButton(x, y, previewEntity, modelAssembly);
    }
    ...
    public void init() {
        ...
        addRenderableWidget(new IconButton(this.guiLeft + 308, this.guiTop + 5, 18, 18, 0, 0, button6 -> {
            if (this.category != Category.STAR) {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if (0 <= size && size < this.sortedModelKeys.size()) {
                ...
                addRenderableWidget(createModelButton(slotX, slotY, previewEntity, modelAssembly2));
            }
        }
    }
