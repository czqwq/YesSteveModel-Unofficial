...
import com.gts.ysmu.client.model.ModelAssemblyFactory;
import com.gts.ysmu.client.gui.IGuiWidget;
import com.gts.ysmu.client.texture.OuterFileTexture;
import com.gts.ysmu.client.upload.IResourceLocatable;
import com.gts.ysmu.model.ServerModelManager;
import com.gts.ysmu.network.NetworkHandler;
import com.gts.ysmu.network.message.C2SModelSyncPayload;
import com.gts.ysmu.resource.*;
import com.gts.ysmu.resource.pojo.RawYsmModel;
import com.gts.ysmu.resource.models.ModelPackData;
...
public class ClientModelManager {
    ...
    public static class ServerModelContext {
        ...
        public ServerModelContext(long hash1, long hash2, String modelId, int isCustomSkinModel, int version) {
    ...
    private static void handlePacket03(YSMByteBuf buf) throws Exception {
        ...
        for (int i = 0; i < unkSize; i++) {
            ...
            ServerModelContext ctx = new ServerModelContext(hash1, hash2, modelId, isCustomSkinModel, version);
            ...
            if (YSMClientCache.verifyFileContent(cachedFile, hash1, hash2)) {
                ...
                parseAndLoadModel(decompressed, modelId);
            } else {
...
//        }
    ...
    private static void handlePacket05(YSMByteBuf buf) throws Exception {
        ...
        if (ctx.bytesReceived >= totalSize) {
            ...
            parseAndLoadModel(decompressed, ctx.modelId);
    ...
    private static void parseAndLoadModel(byte[] decompressed, String modelId) {
        try {
//            if (true) return;
            // IR
            ...
            try (YSMBinaryDeserializer deserializer = new YSMBinaryDeserializer(decompressed, 32)) {
                ...
                onModelDataReceived(parsedBundle, modelId, false);
            }
        } catch (Exception e) {
...
//        localModelContext = null;
    ...
    private static void onModelContextsUpdated(String[] removedModelIds, String[] previousModelIds, String[] updatedModelIds) {
        Minecraft.getInstance().execute(() -> {
    ...
    private static void onModelDataReceived(@Nullable ClientModelInfo parsedBundle, String modelId, boolean isPrimary) throws Exception {
        if (isPrimary) {
            ...
            runPendingModelCallback();
            processModelData(parsedBundle, modelId, false);
        }
    }
    ...
    public static void processModelData(@Nullable ClientModelInfo parsedBundle, String modelId, boolean isPrimary) {
        if (parsedBundle != null) {
            try {
                ModelAssembly runtimeModel = ModelAssemblyFactory.buildAssembly(parsedBundle, isPrimary);
